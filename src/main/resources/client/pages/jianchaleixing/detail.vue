	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('jianchaleixing', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jianchaleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','jianchaleixing');
			...
			async init(){
				...
				let res = await this.$api.info('jianchaleixing', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussjianchaleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
